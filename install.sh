...
if ! need_cmd curl || ! need_cmd jq || ! need_cmd tar; then
  ...
  apt-get update -y
  apt-get install -y curl jq tar whiptail
fi
...
echo "Log    : ${LOG_FILE}"
echo
# --- Mode selection (Install vs Update) ---
# Install  = clean install (löscht /opt/jkef-trading-bot komplett inkl. .env + data)
# Update   = Update (behält /opt/jkef-trading-bot/.env und /opt/jkef-trading-bot/data)
INSTALL_MODE="update"

if command -v whiptail >/dev/null 2>&1; then
  # whiptail needs a real TTY
  INSTALL_MODE="$(whiptail --title "JKEF Installer" \
    --menu "Bitte Aktion wählen" 15 70 2 \
    "install" "Neu installieren (löscht /opt/jkef-trading-bot inkl. .env + data)" \
    "update"  "Update (behält /opt/jkef-trading-bot/.env und /opt/jkef-trading-bot/data)" \
    3>&1 1>&2 2>&3 </dev/tty || true)"
  [[ -n "${INSTALL_MODE}" ]] || die "Abgebrochen."
else
  echo "Aktion wählen:"
  echo "  1) Install (Neuinstallation, löscht /opt/jkef-trading-bot inkl. .env + data)"
  echo "  2) Update  (behält /opt/jkef-trading-bot/.env und /opt/jkef-trading-bot/data)"
  read -r -p "Auswahl [2]: " _sel < /dev/tty
  _sel="${_sel:-2}"
  case "$_sel" in
    1) INSTALL_MODE="install" ;;
    2) INSTALL_MODE="update" ;;
    *) die "Ungültige Auswahl: $_sel" ;;
  esac
fi
...
# Wenn Neuinstallation gewählt: /opt-Installation komplett löschen
if [[ "${INSTALL_MODE}" == "install" ]]; then
  log "Neuinstallation: lösche /opt/${TARGET_DIR_NAME} (inkl. .env + data) ..."
  rm -rf "/opt/${TARGET_DIR_NAME}" || true
fi
...
export JKEF_INSTALL_MODE="$INSTALL_MODE"

# Token an den Bot-Installer durchreichen (Token soll nur EINMAL eingegeben werden)
export JKEF_GH_TOKEN="$TOKEN"
export GH_TOKEN="$TOKEN"  # kompatibel, falls intern noch GH_TOKEN verwendet wird
